...
unsigned long Stepper_update(Stepper *stepper, long unsigned int step_ticks) {
    ...
    unsigned long accumulation = step_ticks % stepper->step_skip;
    unsigned long pure_steps = step_ticks / stepper->step_skip;
    stepper->stack += pure_steps;

    stepper->accumulator += accumulation;
    unsigned long accumulated_steps = stepper->accumulator / stepper->step_skip;
...
unsigned long Stepper_clear(Stepper *stepper) {
    unsigned long stack = stepper->stack;
    stepper->stack = 0;
    return stack;
}
